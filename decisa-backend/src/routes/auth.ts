...
function setAuthCookies(
    res: Parameters<Router["post"]>[1] extends never ? never : any,
    accessToken: string,
    refreshToken: string,
) {
    ...
    try {
        ...
        const resultTransaction = await prisma.$transaction(
            async (tx) => {
                ...
                const realRefreshTokenHash =
                    hashRefreshToken(realRefreshToken);
        ...
        res.cookie(
            "refreshToken",
            resultTransaction.refreshToken,
            {
                httpOnly: true,
                secure:
                    process.env.NODE_ENV === "production",
        ...
        return res.status(201).json({
            user: {
                ...
                displayName:
                    resultTransaction.user.displayName,
            },
        });
    } catch (error) {
